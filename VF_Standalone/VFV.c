...
#undef __FUNCT__
#define __FUNCT__ "VF_RHSAT2VSurface3D_local"
/*
...
extern PetscErrorCode VF_VAssembly3D(Mat K,Vec RHS,VFFields *fields,VFCtx *ctx)
{
  ...
  */
  ...
  ierr = DAVecRestoreArray(ctx->daScal,RHS_localVec,&RHS_array);CHKERRQ(ierr);
...
extern PetscErrorCode VF_IrrevApplyEQ(Mat K,Vec RHS,Vec V,Vec VIrrev,VFProp *vfprop,VFCtx *ctx)
{
  ...
  ierr = DAVecGetArray(ctx->daScal,VIrrev,&VIrrev_array);CHKERRQ(ierr);        
  ierr = DAVecGetArray(ctx->daScal,RHS,&RHS_array);CHKERRQ(ierr);        
  ...
  */
  ...
  ierr = MatZeroRowsStencil(K,myirrevnum,row,one);CHKERRQ(ierr);
  ierr = PetscFree(row);CHKERRQ(ierr);

  ierr = DAVecRestoreArrayDOF(ctx->daScal,V,&V_array);CHKERRQ(ierr);
  ierr = DAVecRestoreArrayDOF(ctx->daScal,VIrrev,&VIrrev_array);CHKERRQ(ierr);
  ierr = DAVecRestoreArrayDOF(ctx->daScal,RHS,&RHS_array);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
#undef __FUNCT__
#define __FUNCT__ "DistanceToDisk"
/*
  DistanceToDisk: Compute the distance from a point with coordinate x and a disk with 
  center xc, normal vector given by the angles theta, phi and radius r
...
extern PetscReal DistanceToDisk(PetscReal *dist,PetscReal *x,PetscReal *xc,PetscReal phi,PetscReal theta,PetscReal r)
{
  ...
  */
  xdotn = (x[0]-xc[0])*n[0] + (x[1]-xc[1])*n[1] + (x[2]-xc[2])*n[2];
  tau[0] = (x[0]-xc[0]) - xdotn*n[0];
  tau[1] = (x[1]-xc[1]) - xdotn*n[1];
  tau[2] = (x[2]-xc[2]) - xdotn*n[2];
  l = sqrt(tau[0]*tau[0] + tau[1]*tau[1] + tau[2]*tau[2]);
  
  if (l < r) {
    *dist = sqrt(xdotn*xdotn);
  } else {
    *dist  = sqrt(pow(x[0] - xc[0] - tau[0] / l * r,2) +  
                  pow(x[1] - xc[1] - tau[1] / l * r,2) + 
                  pow(x[2] - xc[2] - tau[2] / l * r,2));
  } 
  PetscFunctionReturn(0);
}
