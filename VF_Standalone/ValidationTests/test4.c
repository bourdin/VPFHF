  ...
  switch (orientation) {
    ...
    case 1:
      ...
      for (k = zs; k < zs+zm; k++) {
        for (j = ys; j < ys+ym; j++) {
          for (i = xs; i < xs+xm; i++) { 
            if (((i == nx/2) || (i == nx/2-1)) && (PetscAbs(coords_array[k][j][i][1]-(BBmin[1]+BBmax[1])/2.) < length)) {
              v_array[k][j][i] = 0.;
            }
          }
        }
    ...
    case 4:
      ...
      for (k = zs; k < zs+zm; k++) {
        for (j = ys; j < ys+ym; j++) {
          for (i = xs; i < xs+xm; i++) { 
            if (((k == nz/2) || (k == nz/2-1)) && (PetscAbs(coords_array[k][j][i][1]-(BBmin[1]+BBmax[1])/2.) < length)) {
              v_array[k][j][i] = 0.;
            }
          }
        }
    ...
    case 5:
      ...
      for (k = zs; k < zs+zm; k++) {
        for (j = ys; j < ys+ym; j++) {
          for (i = xs; i < xs+xm; i++) { 
            if (((k == nz/2) || (k == nz/2-1)) && (PetscAbs(coords_array[k][j][i][0]-(BBmin[0]+BBmax[0])/2.) < length)) {
              v_array[k][j][i] = 0.;
            }
          }
        }
