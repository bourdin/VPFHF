...
def plottest2(vf_prefix,gmrs_file):
    OpenDatabase(gmrs_file, 0)
    
    AddPlot("Mesh", "mesh", 1, 0)
    DefineVectorExpression("DISPL", "{<DISPX >,<DISPY >,<DISPZ >}")
    AddOperator("Displace", 0)
    SetActivePlots(0)
    DisplaceAtts = DisplaceAttributes()
    DisplaceAtts.factor = 200.
    DisplaceAtts.variable = "DISPL"
    SetOperatorOptions(DisplaceAtts, 0)
    
    MeshAtts = MeshAttributes()
    MeshAtts.legendFlag = 0
    MeshAtts.meshColor = (204, 255, 204, 255)
    MeshAtts.meshColorSource = MeshAtts.MeshCustom
    SetPlotOptions(MeshAtts)
    
    
    OpenDatabase(vf_prefix+".xmf", 0)
    AddPlot("Mesh", vf_prefix+".00001.h5", 1, 0)
    ...
    TransformAtts.translateY = -float(bb[3])/2.
    ...
    import os.path
    outfile = vf_prefix
    if os.path.exists(vf_prefix+'-0000.png'):
        os.remove(vf_prefix+'-0000.png')
    
    SaveWindowAtts = SaveWindowAttributes()
    SaveWindowAtts.outputToCurrentDirectory = 1
    SaveWindowAtts.outputDirectory = "."
    SaveWindowAtts.fileName = vf_prefix+'-'
...
vf_prefix = sys.argv[-2]
gmrs_file = sys.argv[-1]
plottest2(vf_prefix,gmrs_file)
sys.exit()
