...
/*
  Comments:
    ...
    RETURN VALUES! What is the point of computing a quantity if it is not
...
extern PetscErrorCode CrackOpeningDisplacement(VFCtx *ctx,VFFields *fields)
{
  ...
  ierr = PetscPrintf(PETSC_COMM_WORLD,"#        Volume change calculated gradv.u and volumetric strain     \n");CHKERRQ(ierr);
  ierr = PetscPrintf(PETSC_COMM_WORLD,"#        Volume change using gradv .u = %f\t        \n",Vol_change_total1);CHKERRQ(ierr);
  ierr = PetscPrintf(PETSC_COMM_WORLD,"#        Volume change using volumetric strain = %f\t       \n\n",Vol_change_total2);CHKERRQ(ierr);

  ierr = DMDAVecRestoreArrayDOF(ctx->daVect,displ_local,&displ_array);CHKERRQ(ierr);
  ierr = DMRestoreLocalVector(ctx->daVect,&displ_local);CHKERRQ(ierr);
  
  ierr = DMDAVecRestoreArray(ctx->daScal,vfield_local,&vfield_array);CHKERRQ(ierr);
  ierr = DMRestoreLocalVector(ctx->daScal,&vfield_local);CHKERRQ(ierr);
...
/*
  Comments:
    ...
    PetscErrorCode VolumetricCrackOpening3D_local(PetscReal *VolumetricCrackOpening_local,
                                                  ...
                                                  PetscInt ei,
                                                  CartFE_Element3D *e);
    in order to match that of ElasticEnergyDensity3D_local in VFU.c
*/
extern PetscErrorCode ComputeXYZOpening(CartFE_Element3D *e,PetscInt ei,PetscInt ej,PetscInt ek,PetscReal hx,PetscReal hy,PetscReal hz,PetscReal ****displ_array,PetscReal ***vfield_array,PetscReal ****perm_array)
{
  ...
  PetscFunctionBegin;
  ierr = PetscMalloc3(e->ng,PetscReal,&udispl_loc,
                      e->ng,PetscReal,&vdispl_loc,
                      e->ng,PetscReal,&wdispl_loc);CHKERRQ(ierr);
  ierr = PetscMalloc3(e->ng,PetscReal,&dx_vfield_loc,
                      e->ng,PetscReal,&dy_vfield_loc,
                      e->ng,PetscReal,&dz_vfield_loc);CHKERRQ(ierr);
  ierr = PetscMalloc3(e->ng,PetscReal,&du_loc,
                      e->ng,PetscReal,&dv_loc,
                      e->ng,PetscReal,&dw_loc);CHKERRQ(ierr);
  ...
  ierr = PetscFree3(du_loc,dv_loc,dw_loc);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
extern PetscErrorCode CellToNodeInterpolation(DM dm,Vec node_vec,Vec cell_vec,VFCtx *ctx)
{
  ...
  ierr = DMGetGlobalVector(ctx->daScal,&volume);CHKERRQ(ierr);
  ...
  ierr = DMRestoreGlobalVector(ctx->daScal,&volume);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
