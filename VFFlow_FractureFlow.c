...
extern PetscErrorCode MixedFractureFlowSolverInitialize(VFCtx *ctx, VFFields *fields)
{
	...
	if (comm_size == 1) {
		ierr = DMCreateMatrix(ctx->daFlow,MATSEQAIJ,&ctx->KFracVelP);CHKERRQ(ierr);
		ierr = DMCreateMatrix(ctx->daFlow,MATSEQAIJ,&ctx->KFracVelPlhs);CHKERRQ(ierr);
	ierr = DMCreateMatrix(ctx->daFlow,MATSEQAIJ,&ctx->JacFracVelP);CHKERRQ(ierr);
	} else {
		ierr = DMCreateMatrix(ctx->daFlow,MATMPIAIJ,&ctx->KFracVelP);CHKERRQ(ierr);
		ierr = DMCreateMatrix(ctx->daFlow,MATSEQAIJ,&ctx->KFracVelPlhs);CHKERRQ(ierr);
	ierr = DMCreateMatrix(ctx->daFlow,MATMPIAIJ,&ctx->JacFracVelP);CHKERRQ(ierr);
	}
