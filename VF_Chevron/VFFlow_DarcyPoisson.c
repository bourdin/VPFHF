...
#undef __FUNCT__
#define __FUNCT__ "VF_FlowPoisson_MatPAssembly3D_local"
/* 
   VF_FlowPoisson_MatPAssembly3D_local
*/

extern PetscErrorCode VF_FlowPoisson_MatPAssembly3D_local(PetscReal *Mat_local,ResProp *resprop,PetscInt ek,PetscInt ej,PetscInt ei,CartFE_Element3D *e)
{
...
#undef __FUNCT__
#define __FUNCT__ "VF_FlowPoisson_MatPAssembly3D"
/* 
   VF_FlowPoisson_MatPAssembly3D
*/
extern PetscErrorCode VF_FlowPoisson_MatPAssembly3D(Mat K,Vec RHS,VFFields *fields,VFCtx *ctx)
{
  PetscErrorCode ierr;
  PetscInt       xs,xm,nx;
  PetscInt       ys,ym,ny;
    ...
    for (ej = ys; ej < ys+ym; ej++) {
      for (ei = xs; ei < xs+xm; ei++) {
        ...
        */
        ...
        ierr = VF_FlowPoisson_MatPAssembly3D_local(K_local,&ctx->resprop,ek,ej,ei,&ctx->e3D);
...
#undef __FUNCT__
#define __FUNCT__ "VFFlow_DarcyPoisson"
/* 
...
extern PetscErrorCode VFFlow_DarcyPoisson(VFCtx *ctx, VFFields *fields)
{
  ...
  ierr = VF_FlowPoisson_MatPAssembly3D(ctx->KP,ctx->RHSP,fields,ctx);CHKERRQ(ierr);
  if (ctx->verbose > 1) {
