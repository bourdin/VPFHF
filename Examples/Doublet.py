...
def mygetenv(Dict,key,defaultvalue=None):
    '''Adds an entry to a dictionary based on the environment variable
       with same name. Accepts an optional default value.'''
    import os
    tmp = os.getenv(key)
    if tmp == None:
...
def Dictwritetxt(D,filename):
    ''' Saves a dictionary in a primitive text format'''
    f = open(filename,'a')
    K = D.keys()
    K.sort()
    for key in K:
...
def DictwriteJSON(D,filename):
    ...
    try:
        ...
        jsonfile.write(json.dumps(D,indent=2))
        jsonfile.flush()
        jsonfile.close()
    except ImportError:
