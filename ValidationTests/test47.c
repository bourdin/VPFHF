/*
 ...
 ./test47 -n 51,51,2 -l 2,2,0.01  -theta 1  -timevalue 0.01 -maxtimestep 50 -condx 0.01 -condy 0.01 -velx 0.8 -vely 0.8
 
 ./test47 -n 101,101,2 -l 2,2,0.01  -theta 1  -timevalue 0.001 -maxtimestep 200 -condx 0.01 -condy 0.01 -velx 01 -vely 01
...
int main(int argc,char **argv)
{
    ...
    ierr = PetscOptionsGetReal(NULL,NULL,"-condx",&kx,NULL);CHKERRQ(ierr);
    ierr = PetscOptionsGetReal(NULL,NULL,"-condy",&ky,NULL);CHKERRQ(ierr);
    ierr = PetscOptionsGetReal(NULL,NULL,"-velx",&ux,NULL);CHKERRQ(ierr);
    ierr = PetscOptionsGetReal(NULL,NULL,"-vely",&uy,NULL);CHKERRQ(ierr);
    ...
    for (ctx.timestep = 1; ctx.timestep < ctx.maxtimestep; ctx.timestep++){
        ...
        for (k = zs; k < zs+zm; k++) {
            for (j = ys; j < ys+ym; j++) {
                for (i = xs; i < xs+xm; i++) {
                    Tbc_array[k][j][i] = 1/(4*ctx.timestep*ctx.timevalue+1)*exp(-(pow((hx*i-ux*ctx.timestep*ctx.timevalue-0.5),2))/(kx*(4*ctx.timestep*ctx.timevalue+1))-(pow((hy*j-uy*ctx.timestep*ctx.timevalue-0.5),2))/(ky*(4*ctx.timestep*ctx.timevalue+1)));
                }
            }
        }
