...
int main(int argc,char **argv)
{
  ...
  PetscInt        num = 10;
  PetscReal       errP=1e+10,errV=1e+10,errVP=1e+10;
  Vec             Pold,Pold1,Vold;
  PetscReal       pmax,vmax;
  ...
  ctx.bcFracQ[1].face[Y0] = FIXED;
  ctx.bcFracQ[1].face[Y1] = FIXED;
  ...
  ctx.insitumax[2] = ctx.insitumin[2] = -2.e-3;
  ...
  ierr = PetscViewerASCIIPrintf(viewer,"Time step \t Time \t Pressure \t VolumeInj  \t FracVolume \t LeakOffVolume \n");CHKERRQ(ierr);
  ...
  for(i = 1; i < num; i++){
    ...
    while ((errV >= tolV && errVP >= tolV && altminit < 300)){
      ...
      while (errP >= tolP){
        ...
        ierr = VecMax(Pold,PETSC_NULL,&pmax);CHKERRQ(ierr);
        ierr = PetscPrintf(PETSC_COMM_WORLD," Pressure norm = %e, max pressure error = %e\n", errP,pmax);CHKERRQ(ierr);
...
//      ierr = VecMax(fields.V,PETSC_NULL,&vmax);CHKERRQ(ierr);
    ...
    ierr = PetscViewerASCIIPrintf(viewer,"%d \t %e \t %e \t %e \t %e \t %e \n",i,i*ctx.flowprop.timestepsize,pmax,i*Q_inj,ctx.CrackVolume,ctx.LeakOffRate);CHKERRQ(ierr);
