...
int main(int argc,char **argv)
{
  ...
  */
  ...
  ierr = PetscViewerASCIIPrintf(ctx.energyviewer,"#l,p,volume,Elastic Energy,InsituWork,Pressure Work,Total Energy\n");CHKERRQ(ierr);
  for (ctx.timestep = 0; ctx.timestep < ctx.maxtimestep; ctx.timestep++) {
    if (ctx.maxtimestep > 1) {
      ctx.timevalue = (PetscReal) ctx.timestep * (rmax - rmin) / (ctx.maxtimestep - 1.) + rmin;
    } else {
      ctx.timevalue = rmin;
    }
    ierr = PetscPrintf(PETSC_COMM_WORLD,"Solving for l = %g\n",ctx.timevalue);CHKERRQ(ierr);
    ierr = VecSet(fields.pressure,p);CHKERRQ(ierr);
    ctx.pennycrack[0].r = ctx.timevalue;
    
    ierr = VecSet(fields.V,1.);CHKERRQ(ierr);
    if (ctx.vfprop.atnum == 1) {
        ierr = VFPennyCrackBuildVAT1(fields.V,&(ctx.pennycrack[0]),&ctx);CHKERRQ(ierr);
    } else {
        ierr = VFPennyCrackBuildVAT2(fields.V,&(ctx.pennycrack[0]),&ctx);CHKERRQ(ierr);
    }
    ...
    */
    ...
    ierr = PetscViewerASCIIPrintf(ctx.energyviewer,"%e \t%e \t%e \t%e \t%e \t%e \t%e\n",ctx.timevalue,p,crackVolume,ctx.ElasticEnergy,ctx.InsituWork,ctx.PressureWork,ctx.ElasticEnergy-ctx.InsituWork-ctx.PressureWork);CHKERRQ(ierr);
  }
