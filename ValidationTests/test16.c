  ...
  */
  ...
  ierr = PetscSNPrintf(filename,FILENAME_MAX,"%s.pres",ctx.prefix);CHKERRQ(ierr);
	...
	for (ctx.timestep = 0; ctx.timestep < ctx.maxtimestep; ctx.timestep++){
      ...
      ierr = PetscPrintf(PETSC_COMM_WORLD,"      Rel. change on p: %e (tol %e)\n",errP,p_epsilon);CHKERRQ(ierr);
      ierr = VecAXPY(fields.U,p,U_1);CHKERRQ(ierr);

      ctx.CrackVolume = targetVol;
			ierr = PetscPrintf(PETSC_COMM_WORLD,"      vol_s: %e vol_1 %e volume %e \n",vol_s,vol_1,ctx.CrackVolume);
			...
			altminit++;
		} while (errP >= p_epsilon && altminit <= ctx.altminmaxit);
