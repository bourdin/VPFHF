...
int main(int argc,char **argv)
{
  ...
  ierr = PetscPrintf(PETSC_COMM_WORLD,"  Computing initial time step solution,F = %e, F*lx = %e..................\n", F,F*lx);CHKERRQ(ierr);
  displ = lz*(1.+ctx.matprop[0].nu)*(2*(1.-ctx.matprop[0].nu)*F+ctx.matprop[0].beta*SumnIntegral*(1.-2*ctx.matprop[0].nu))/(lx*ly*ctx.matprop[0].E);
