/*
 test46.c: 2D Heat problem with heat source term = 1. All temperature boundary condition. [Homomogeneous temperature boundaries.]
 (c) 2010-2013 Chukwudi Chukwudozie cchukw1@tigers.lsu.edu
 
 ./test46 -n 51,5,2 -l 1,0.4,0.01  -theta 1  -timestepsize 0.01 -maxtimestep 100 -cond 1
 ./test46 -n 51,5,2 -l 1,0.4,0.01  -theta 1  -timestepsize 0.05 -maxtimestep 150 -cond 0.1
 ./test46 -n 51,51,2 -l 1,1,0.01  -theta 1  -timestepsize 0.05 -maxtimestep 150 -cond 0.01
...
{	
	...
	PetscReal		****vel_array;
	PetscReal		ux = 01.,condctvty = 0.0, src = 1.;
...
//	ctx.flowsolver = HEATSOLVER_SNESFEM;
	...
	ierr = DMDAVecGetArray(ctx.daVect,ctx.HeatFluxBCArray,&heatfluxbc_array);CHKERRQ(ierr);
	...
	for (k = zs; k < zs+zm; k++) {
		for (j = ys; j < ys+ym; j++) {
			for (i = xs; i < xs+xm; i++) {
				...
				vel_array[k][j][i][0] = ux/(pow(2,0.5));
				vel_array[k][j][i][1] = ux/(pow(2,0.5));
