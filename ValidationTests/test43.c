/*
 test43.c: 3D SNES. Single well problem. Source term implemented as dirac function and analytical solution is 3D Green's function. All pressure boundary condition.
 (c) 2010-2012 Chukwudi Chukwudozie cchukw1@tigers.lsu.edu
 
 ./test43  -n 41,41,41 -l 1,1,1 -maxtimestep 1 timestepsize 1 -theta 1 -nc 1 -w0_coords 0.50001,0.50001,0.50001 -w0_Qw 1 -w0_constraint Rate -w0_rw 0.1 -w0_type INJECTOR -m_inv 0.
 */
...
//	ctx.flowsolver = FLOWSOLVER_KSPMIXEDFEM;
	...
	for (k = zs; k < zs+zm; k++) {
		...
		for(k = zs; k < zs+zm; k++){
			for(j = ys; j < ys+ym; j++){
				...
				flowbc_array[k][j][0][3] = -1./(4.*pi*dist);
				...
				flowbc_array[k][j][nx-1][3] = -1./(4.*pi*dist);
	...
	for(k = zs; k < zs+zm; k++){
		for(i = xs; i < xs+xm; i++){
			...
			flowbc_array[k][0][i][3] = -1./(4.*pi*dist);
			...
			flowbc_array[k][ny-1][i][3] = -1./(4.*pi*dist);
	...
	for(j = ys; j < ys+ym; j++){
		for(i = xs; i < xs+xm; i++){
			...
			flowbc_array[0][j][i][3] = -1./(4.*pi*dist);
			...
			flowbc_array[nz-1][j][i][3] = -1./(4.*pi*dist);
