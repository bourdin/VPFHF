/*
 ...
 mpiexec -np 2 ./test27 -n 11,11,11 -l 1,1,1 -uniformfluidsources -flowsolver FLOWSOLVER_SNESSTANDARDFEM -M_inv 0 -P_X0_BC FIXED -P_X1_BC FIXED -P_Y0_BC FIXED -P_Y1_BC FIXED -P_Z0_BC FIXED -P_Z1_BC FIXED -kx 1. -ky 1. -kz 1 -g 0,0,0 -rhof 0. -mu 1
 
 ./test27 -n 11,11,11 -l 1,1,1 -m_inv 0 -uniformfluidsources -ts_type beuler -ts_dt 1 -ts_max_steps 2 -flowsolver FLOWSOLVER_TSMIXEDFEM
...
{	
  ...
  PetscReal   vol,vol1,vol2,vol3,vol4,vol5;
	...
	for (k = zs; k < zs+zm; k++) {
		for (j = ys; j < ys+ym; j++) {
			for (i = xs; i < xs+xm; i++) {
        presbc_array[k][j][i] = (cos(PETSC_PI*coords_array[k][j][i][0]/lx)*cos(PETSC_PI*coords_array[k][j][i][1]/ly)*cos(PETSC_PI*coords_array[k][j][i][2]/lz))/(3.*PETSC_PI*PETSC_PI);
			}
		}
	} 
	for (k = zs; k < zs+zm; k++) {
		for (j = ys; j < ys+ym; j++) {
			for (i = xs; i < xs+xm; i++) {
        src_array[k][j][i] = 1./ctx.flowprop.mu*cos(PETSC_PI*coords_array[k][j][i][2]/lz)*cos(PETSC_PI*coords_array[k][j][i][1]/ly)*cos(PETSC_PI*coords_array[k][j][i][0]/lx)/3.*((1./(lx*lx))+(1./(ly*ly))+(1./(lz*lz)));
			}
		}
	} 
