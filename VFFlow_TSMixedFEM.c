...
extern PetscErrorCode MixedFlowFEMTSSolve(VFCtx *ctx,VFFields *fields)
{
  ...
  ierr = TSSetTime(ctx->tsVelP,ctx->current_time);CHKERRQ(ierr);
  ierr = TSSetTimeStep(ctx->tsVelP,ctx->timevalue);CHKERRQ(ierr);
  ierr = TSSetMaxSteps(ctx->tsVelP,ctx->maxtimestep);CHKERRQ(ierr);
  ierr = TSSetMaxTime(ctx->tsVelP,ctx->timevalue);CHKERRQ(ierr);
...
//	}
  ...
  ierr = TSGetStepNumber(ctx->tsVelP,&ctx->timestep);CHKERRQ(ierr);
  ...
  for (k = zs; k < zs+zm; k++) {
    for (j = ys; j < ys+ym; j++)
      for (i = xs; i < xs+xm; i++) {
        ...
        for (c = 0; c < veldof; c++) vel_array[k][j][i][c] =  VelnPress_array[k][j][i][c];
      }
  }
  ...
  for (ek = zs; ek < zs+zm; ek++) {
    for (ej = ys; ej < ys+ym; ej++)
      for (ei = xs; ei < xs+xm; ei++) {
        ...
        for (l = 0; l < nrow*nrow; l++) Klhs_local[l] = -1.*m_inv_array[ek][ej][ei]*Klhs_local[l];
        for (c = 0; c < veldof; c++) {
        ...
        for (c = 0; c < veldof; c++) {
          ...
          for (l = 0,k = 0; k < ctx->e3D.nphiz; k++)
            for (j = 0; j < ctx->e3D.nphiy; j++)
              for (i = 0; i < ctx->e3D.nphix; i++,l++) RHS_array[ek+k][ej+j][ei+i][c] += RHS_local[l];
        }
        ...
        for (l = 0,k = 0; k < ctx->e3D.nphiz; k++)
          for (j = 0; j < ctx->e3D.nphiy; j++)
            for (i = 0; i < ctx->e3D.nphix; i++,l++) RHS_array[ek+k][ej+j][ei+i][3] += RHS_local[l];
          ...
          if (ctx->bcP[0].face[face] == FIXED) {
            ...
            for (l=0,k = 0; k < ctx->e2D.nphix; k++)
              for (j = 0; j < ctx->e2D.nphiy; j++)
                for (i = 0; i < ctx->e2D.nphiz; i++, l++) RHS_array[ek+k][ej+j][ei+i][0] -= RHS_local[l];
          }
        }
        if (ei == nx-1) {
          /*					 Face X1		*/
          ...
          if (ctx->bcP[0].face[face] == FIXED) {
            ...
            for (l=0,k = 0; k < ctx->e2D.nphix; k++)
              for (j = 0; j < ctx->e2D.nphiy; j++)
                for (i = 0; i < ctx->e2D.nphiz; i++, l++) RHS_array[ek+k][ej+j][ei+1][0] += RHS_local[l];
          }
        }
        if (ej == 0) {
          /*					 Face Y0		*/
          ...
          if (ctx->bcP[0].face[face] == FIXED) {
            ...
            for (l=0,k = 0; k < ctx->e2D.nphiy; k++)
              for (j = 0; j < ctx->e2D.nphiz; j++)
                for (i = 0; i < ctx->e2D.nphix; i++, l++) RHS_array[ek+k][ej+j][ei+i][1] -= RHS_local[l];
          }
        }
        if (ej == ny-1) {
          /*					 Face Y1		*/
          ...
          if (ctx->bcP[0].face[face] == FIXED) {
            ...
            for (l=0,k = 0; k < ctx->e2D.nphiy; k++)
              for (j = 0; j < ctx->e2D.nphiz; j++)
                for (i = 0; i < ctx->e2D.nphix; i++, l++) RHS_array[ek+k][ej+1][ei+i][1] += RHS_local[l];
          }
        }
        if (ek == 0) {
          /*					 Face Z0		*/
          ...
          if (ctx->bcP[0].face[face] == FIXED) {
            ...
            for (l=0,k = 0; k < ctx->e2D.nphiz; k++)
              for (j = 0; j < ctx->e2D.nphiy; j++)
                for (i = 0; i < ctx->e2D.nphix; i++, l++) RHS_array[ek+k][ej+j][ei+i][2] -= RHS_local[l];
          }
        }
        if (ek == nz-1) {
          /*					 Face Z1		*/
          ...
          if (ctx->bcP[0].face[face] == FIXED) {
            ...
            for (l=0,k = 0; k < ctx->e2D.nphiz; k++)
              for (j = 0; j < ctx->e2D.nphiy; j++)
                for (i = 0; i < ctx->e2D.nphix; i++, l++) RHS_array[ek+1][ej+j][ei+i][2] += RHS_local[l];
          }
        }
      }
  }
            ...
            if (
              ...
              if (ctx->well[w_no].condition == RATE) {
                ...
                for (l = 0,k = 0; k < ctx->e3D.nphiz; k++) {
                  for (j = 0; j < ctx->e3D.nphiy; j++)
                    for (i = 0; i < ctx->e3D.nphix; i++,l++) {
                      if (ctx->well[w_no].type == INJECTOR) RHS_array[ek+k][ej+j][ei+i][3] -= RHS_local[l]/w_no2;
                      else if (ctx->well[w_no].type == PRODUCER) RHS_array[ek+k][ej+j][ei+i][3] -= -RHS_local[l]/w_no2;
                    }
                }
  ...
  for (c = 0; c < dof; c++) {
    if (xs == 0) {
      ...
      if (bcQ[c].face[X0] == FIXED)
        for (k = zs; k < zs+zm; k++)
          for (j = ys; j < ys+ym; j++) RHS_array[k][j][i][c] = velbc_array[k][j][i][c];
    }
    if (xs+xm == nx) {
      i = nx-1;
    ...
    if (ys == 0) {
      ...
      if (bcQ[c].face[Y0] == FIXED)
        for (k = zs; k < zs+zm; k++)
          for (i = xs; i < xs+xm; i++) RHS_array[k][j][i][c] = velbc_array[k][j][i][c];
    }
    if (ys+ym == ny) {
      j = ny-1;
    ...
    if (xs == 0 && ys == 0) {
      ...
      if (bcQ[c].edge[X0Y0] == FIXED)
        for (k = zs; k < zs+zm; k++) RHS_array[k][j][i][c] = velbc_array[k][j][i][c];
    }
    if (xs == 0 && ys+ym == ny) {
      j = ny-1;i = 0;
    ...
    if (xs == 0 && ys == 0 && zs == 0) {
      k = 0;j = 0;i = 0;
      if (bcQ[c].vertex[X0Y0Z0] == FIXED) RHS_array[k][j][i][c] = velbc_array[k][j][i][c];
    }
    if (xs+xm == nx && ys == 0 && zs == 0) {
      k = 0;j = 0;i = nx-1;
      if (bcQ[c].vertex[X1Y0Z0] == FIXED) RHS_array[k][j][i][c] = velbc_array[k][j][i][c];
    }
    if (xs == 0 && ys+ym == ny && zs == 0) {
      k = 0;j = ny-1;i = 0;
      if (bcQ[c].vertex[X0Y1Z0] == FIXED) RHS_array[k][j][i][c] = velbc_array[k][j][i][c];
    }
    if (xs+xm == nx && ys+ym == ny && zs == 0) {
      k = 0;j = ny-1;i = nx-1;
      if (bcQ[c].vertex[X1Y1Z0] == FIXED) RHS_array[k][j][i][c] = velbc_array[k][j][i][c];
    }
    if (xs == 0 && ys == 0 && zs+zm == nz) {
      k = nz-1;j = 0;i = 0;
      if (bcQ[c].vertex[X0Y0Z1] == FIXED) RHS_array[k][j][i][c] = velbc_array[k][j][i][c];
    }
    if (xs+xm == nx && ys == 0 && zs+zm == nz) {
      k = nz-1;j = 0;i = nx-1;
      if (bcQ[c].vertex[X1Y0Z1] == FIXED) RHS_array[k][j][i][c] = velbc_array[k][j][i][c];
    }
    if (xs == 0 && ys+ym == ny && zs+zm == nz) {
      k = nz-1;j = ny-1;i = 0;
      if (bcQ[c].vertex[X0Y1Z1] == FIXED) RHS_array[k][j][i][c] = velbc_array[k][j][i][c];
    }
    if (xs+xm == nx && ys+ym == ny && zs+zm == nz) {
      k = nz-1;j = ny-1;i = nx-1;
      if (bcQ[c].vertex[X1Y1Z1] == FIXED) RHS_array[k][j][i][c] = velbc_array[k][j][i][c];
    }
  }
...
extern PetscErrorCode FormInitialSolution(Vec VelnPress,Vec VelnPressBV, VFBC *bcP,VFBC *bcQ, VFCtx *ctx)
{
  ...
  if (xs == 0) {
    ...
    if (bcP[0].face[X0] == FIXED)
      for (k = zs; k < zs+zm; k++)
        for (j = ys; j < ys+ym; j++) IniSol_array[k][j][i][3] = UnPre_array[k][j][i][3];
    else
      for (c = 0; c < dof; c++)
        if (bcQ[c].face[X0] == FIXED)
          for (k = zs; k < zs+zm; k++)
            for (j = ys; j < ys+ym; j++) IniSol_array[k][j][i][c] = UnPre_array[k][j][i][c];
  ...
  if (ys == 0) {
    ...
    if (bcP[0].face[Y0] == FIXED)
      for (k = zs; k < zs+zm; k++)
        for (i = xs; i < xs+xm; i++) IniSol_array[k][j][i][3] = UnPre_array[k][j][i][3];
    else
      for (c = 0; c < dof; c++)
        if (bcQ[c].face[Y0] == FIXED)
          for (k = zs; k < zs+zm; k++)
            for (i = xs; i < xs+xm; i++) IniSol_array[k][j][i][c] = UnPre_array[k][j][i][c];
  ...
  if (xs == 0 && ys == 0) {
    ...
    if (bcP[0].edge[X0Y0] == FIXED)
      for (k = zs; k < zs+zm; k++) IniSol_array[k][j][i][3] = UnPre_array[k][j][i][3];

    else
      for (c = 0; c < dof; c++)
        if (bcQ[c].edge[X0Y0] == FIXED)
          for (k = zs; k < zs+zm; k++) IniSol_array[k][j][i][c] = UnPre_array[k][j][i][c];
  ...
  if (xs == 0 && ys == 0 && zs == 0) {
    k = 0;j = 0;i = 0;
    if (bcP[0].vertex[X0Y0Z0] == FIXED) IniSol_array[k][j][i][3] = UnPre_array[k][j][i][3];
    else
      for (c = 0; c < dof; c++)
        if (bcQ[c].vertex[X0Y0Z0] == FIXED) IniSol_array[k][j][i][c] = UnPre_array[k][j][i][c];
  }
  if (xs+xm == nx && ys == 0 && zs == 0) {
    k = 0;j = 0;i = nx-1;
    if (bcP[0].vertex[X1Y0Z0] == FIXED) IniSol_array[k][j][i][3] = UnPre_array[k][j][i][3];
    else
      for (c = 0; c < dof; c++)
        if (bcQ[c].vertex[X1Y0Z0] == FIXED) IniSol_array[k][j][i][c] = UnPre_array[k][j][i][c];
  }
  //
  if (xs == 0 && ys+ym == ny && zs == 0) {
    k = 0;j = ny-1;i = 0;
    if (bcP[0].vertex[X0Y1Z0] == FIXED) IniSol_array[k][j][i][3] = UnPre_array[k][j][i][3];
    else
      for (c = 0; c < dof; c++)
        if (bcQ[c].vertex[X0Y1Z0] == FIXED) IniSol_array[k][j][i][c] = UnPre_array[k][j][i][c];
  }
  if (xs+xm == nx && ys+ym == ny && zs == 0) {
    k = 0;j = ny-1;i = nx-1;
    if (bcP[0].vertex[X1Y1Z0] == FIXED) IniSol_array[k][j][i][3] = UnPre_array[k][j][i][3];
    else
      for (c = 0; c < dof; c++)
        if (bcQ[c].vertex[X1Y1Z0] == FIXED) IniSol_array[k][j][i][c] = UnPre_array[k][j][i][c];
  }

  //
  if (xs == 0 && ys == 0 && zs+zm == nz) {
    k = nz-1;j = 0;i = 0;
    if (bcP[0].vertex[X0Y0Z1] == FIXED) IniSol_array[k][j][i][3] = UnPre_array[k][j][i][3];
    else
      for (c = 0; c < dof; c++)
        if (bcQ[c].vertex[X0Y0Z1] == FIXED) IniSol_array[k][j][i][c] = UnPre_array[k][j][i][c];
  }
  if (xs+xm == nx && ys == 0 && zs+zm == nz) {
    k = nz-1;j = 0;i = nx-1;
    if (bcP[0].vertex[X1Y0Z1] == FIXED) IniSol_array[k][j][i][3] = UnPre_array[k][j][i][3];
    else
      for (c = 0; c < dof; c++)
        if (bcQ[c].vertex[X1Y0Z1] == FIXED) IniSol_array[k][j][i][c] = UnPre_array[k][j][i][c];
  }
  ...
  if (xs == 0 && ys+ym == ny && zs+zm == nz) {
    k = nz-1;j = ny-1;i = 0;
    if (bcP[0].vertex[X0Y1Z1] == FIXED) IniSol_array[k][j][i][3] = UnPre_array[k][j][i][3];
    else
      for (c = 0; c < dof; c++)
        if (bcQ[c].vertex[X0Y1Z1] == FIXED) IniSol_array[k][j][i][c] = UnPre_array[k][j][i][c];
  }
  if (xs+xm == nx && ys+ym == ny && zs+zm == nz) {
    k = nz-1;j = ny-1;i = nx-1;
    if (bcP[0].vertex[X1Y1Z1] == FIXED) IniSol_array[k][j][i][3] = UnPre_array[k][j][i][3];
    else
      for (c = 0; c < dof; c++)
        if (bcQ[c].vertex[X1Y1Z1] == FIXED) IniSol_array[k][j][i][c] = UnPre_array[k][j][i][c];
  }
