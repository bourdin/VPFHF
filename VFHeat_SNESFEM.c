...
extern PetscErrorCode VF_HeatFEMSNESSolve(VFCtx *ctx,VFFields *fields)
{
	...
	ierr = PetscViewerASCIIOpen(PETSC_COMM_SELF,"JacMatrix1.txt",&viewer);CHKERRQ(ierr);
	ierr = PetscViewerSetFormat(viewer, PETSC_VIEWER_ASCII_INDEX);CHKERRQ(ierr);
	ierr = MatView(ctx->KT,viewer);CHKERRQ(ierr);


	ierr = PetscViewerASCIIOpen(PETSC_COMM_SELF,"RHST.txt",&viewer);CHKERRQ(ierr);
	ierr = PetscViewerSetFormat(viewer, PETSC_VIEWER_ASCII_INDEX);CHKERRQ(ierr);
	ierr = VecView(ctx->RHST,viewer);CHKERRQ(ierr);
	...
	ierr = PetscViewerASCIIOpen(PETSC_COMM_SELF,"JacMatrixKlhs.txt",&viewer);CHKERRQ(ierr);
	ierr = PetscViewerSetFormat(viewer, PETSC_VIEWER_ASCII_INDEX);CHKERRQ(ierr);
	ierr = MatView(ctx->KTlhs,viewer);CHKERRQ(ierr);
	...
	ierr = VecCopy(VecRHS,ctx->RHSTpre);CHKERRQ(ierr);
	
	
	ierr = PetscViewerASCIIOpen(PETSC_COMM_SELF,"RHS2.txt",&viewer);CHKERRQ(ierr);
	ierr = PetscViewerSetFormat(viewer, PETSC_VIEWER_ASCII_INDEX);CHKERRQ(ierr);
	ierr = VecView(ctx->RHST,viewer);CHKERRQ(ierr);
...
extern PetscErrorCode FormSNESHeatIJacobian(SNES snes,Vec T,Mat *Jac,Mat *Jacpre,MatStructure *str,void *user)
{
	...
	*str = DIFFERENT_NONZERO_PATTERN;
	...
	ierr = PetscViewerASCIIOpen(PETSC_COMM_SELF,"JacMatrixinside.txt",&viewer);CHKERRQ(ierr);
	ierr = PetscViewerSetFormat(viewer, PETSC_VIEWER_ASCII_INDEX);CHKERRQ(ierr);
...
extern PetscErrorCode FormSNESHeatIFunction(SNES snes,Vec T,Vec Func,void *user)
{
	...
	ierr = PetscViewerASCIIOpen(PETSC_COMM_SELF,"RHS1inside.txt",&viewer);CHKERRQ(ierr);
	ierr = PetscViewerSetFormat(viewer, PETSC_VIEWER_ASCII_INDEX);CHKERRQ(ierr);
	ierr = VecView(VecRHS,viewer);CHKERRQ(ierr);	

	ierr = VecAXPBY(VecRHS,dt_dot_one_minus_theta,dt_dot_theta,ctx->RHSTpre);CHKERRQ(ierr);	
	ierr = MatMultAdd(ctx->KTlhs,ctx->prevT,VecRHS,VecRHS);CHKERRQ(ierr);	
	ierr = PetscViewerASCIIOpen(PETSC_COMM_SELF,"RHS2inside.txt",&viewer);CHKERRQ(ierr);
	ierr = PetscViewerSetFormat(viewer, PETSC_VIEWER_ASCII_INDEX);CHKERRQ(ierr);
	ierr = VecView(VecRHS,viewer);CHKERRQ(ierr);	

	ierr = VecApplyDirichletBC(VecRHS,ctx->TBCArray,&ctx->bcT[0]);CHKERRQ(ierr);
	
	ierr = PetscViewerASCIIOpen(PETSC_COMM_SELF,"RHS3inside.txt",&viewer);CHKERRQ(ierr);
	ierr = PetscViewerSetFormat(viewer, PETSC_VIEWER_ASCII_INDEX);CHKERRQ(ierr);
	ierr = VecView(VecRHS,viewer);CHKERRQ(ierr);	
	...
	ierr = PetscViewerASCIIOpen(PETSC_COMM_SELF,"RHSFUNCTinside.txt",&viewer);CHKERRQ(ierr);
	ierr = PetscViewerSetFormat(viewer, PETSC_VIEWER_ASCII_INDEX);CHKERRQ(ierr);
	ierr = VecView(Func,viewer);CHKERRQ(ierr);	
	
	ierr = PetscViewerASCIIOpen(PETSC_COMM_SELF,"RHSpreinside.txt",&viewer);CHKERRQ(ierr);
	ierr = PetscViewerSetFormat(viewer, PETSC_VIEWER_ASCII_INDEX);CHKERRQ(ierr);
	ierr = VecView(ctx->RHSTpre,viewer);CHKERRQ(ierr);	
...
//	ierr = MatDuplicate(K,MAT_COPY_VALUES,&M);CHKERRQ(ierr);
//	ierr = MatDuplicate(K,MAT_COPY_VALUES,&N);CHKERRQ(ierr);
//	ierr = MatDuplicate(K,MAT_COPY_VALUES,&C);CHKERRQ(ierr);
//	ierr = MatDuplicate(K,MAT_COPY_VALUES,&Kk);CHKERRQ(ierr);
//	ierr = MatDuplicate(K,MAT_COPY_VALUES,&BigK);CHKERRQ(ierr);
//	ierr = MatZeroEntries(M);CHKERRQ(ierr);
//	ierr = MatZeroEntries(Kk);CHKERRQ(ierr);
	...
	Cp_sol_array = ctx->matprop[0].Cp;
		...
		//	ierr = DMGlobalToLocalBegin(ctx->daScal,,INSERT_VALUES,rho_sol_local);CHKERRQ(ierr);
	...
	ierr = DMGetLocalVector(ctx->daScal,&fluxbc_local);CHKERRQ(ierr);
	ierr = DMGlobalToLocalBegin(ctx->daScal,ctx->HeatFluxBCArray,INSERT_VALUES,fluxbc_local);CHKERRQ(ierr);
	ierr = DMGlobalToLocalEnd(ctx->daScal,ctx->HeatFluxBCArray,INSERT_VALUES,fluxbc_local);CHKERRQ(ierr);
	ierr = DMDAVecGetArray(ctx->daScal,fluxbc_local,&fluxbc_array);CHKERRQ(ierr); 
		...
		for (ej = ys; ej < ys+ym; ej++) {
			for (ei = xs; ei < xs+xm; ei++) {
				...
				ierr = CartFE_Element3DInit(&ctx->e3D,hx,hy,hz);CHKERRQ(ierr);
					//				rhoCp_eff_array[ek][ej][ei]=rho_liq_array[ek][ej][ei]*Cp_liq_array[ek][ej][ei]+rho_liq_array[ek][ej][ei]*Cp_liq_array[ek][ej][ei];
					//				for(c = 0; c < 6; c++){
					//					diffsvty_array[ek][ej][ei][c] = diffsvty_arrays[ek][ej][ei]/rhoCp_eff_array[ek][ej][ei];
					//				}
				...
				ierr = HeatMatC(KC_local,&ctx->e3D,ek,ej,ei,vel_array);CHKERRQ(ierr);
				for (l = 0,k = 0; k < ctx->e3D.nphiz; k++) {
					for (j = 0; j < ctx->e3D.nphiy; j++) {
						for (i = 0; i < ctx->e3D.nphix; i++,l++) {
							row[l].i = ei+i;row[l].j = ej+j;row[l].k = ek+k;row[l].c = 0;
				...
				ierr = MatSetValuesStencil(K,nrow,row,nrow,row,K1_local,ADD_VALUES);CHKERRQ(ierr);
				ierr = MatSetValuesStencil(Klhs,nrow,row,nrow,row,K2_local,ADD_VALUES);CHKERRQ(ierr);
				ierr = HeatMatN(KN_local,&ctx->e3D,ek,ej,ei,vel_array);CHKERRQ(ierr);
				for (l = 0,k = 0; k < ctx->e3D.nphiz; k++) {
					for (j = 0; j < ctx->e3D.nphiy; j++) {
						for (i = 0; i < ctx->e3D.nphix; i++,l++) {
							row[l].i = ei+i;row[l].j = ej+j;row[l].k = ek+k;row[l].c = 0;
				...
				if(ctx->hasHeatSources){
					ierr = VecApplyHeatSourceTerms(RHS_local,RHS1_local,heatsource_array,&ctx->e3D,ek,ej,ei,ctx,vel_array);
					for (l = 0,k = 0; k < ctx->e3D.nphiz; k++) {
						for (j = 0; j < ctx->e3D.nphiy; j++) {
							for (i = 0; i < ctx->e3D.nphix; i++,l++) {								
								RHS_array[ek+k][ej+j][ei+i] += 1.0*(1./rhoCp_eff_array)*(RHS_local[l]-timestepsize*(rho_liq_array*Cp_liq_array/rhoCp_eff_array)*RHS1_local[l]);
							}
						}
					}
				}
				...
				if (ei == 0) {
					...
					if (ctx->bcQT[0].face[face] == VALUE) {
						ierr = VecApplyHeatFluxBC(RHS_local,fluxbc_array,ek,ej,ei,face,&ctx->e2D);CHKERRQ(ierr);
						for (l=0,k = 0; k < ctx->e2D.nphix; k++){
							for (j = 0; j < ctx->e2D.nphiy; j++) {
								for (i = 0; i < ctx->e2D.nphiz; i++, l++) {
									RHS_array[ek+k][ej+j][ei+i] +=  -1.0*(1./rhoCp_eff_array)*RHS_local[l];
								}
							}
						}
					}
				...
				if (ei == nx-1) {
					...
					if (ctx->bcQT[0].face[face] == VALUE) {
						...
						for (l=0,k = 0; k < ctx->e2D.nphix; k++){
							for (j = 0; j < ctx->e2D.nphiy; j++) {
								for (i = 0; i < ctx->e2D.nphiz; i++, l++) {
									RHS_array[ek+k][ej+j][ei+1] +=  -1.0*(1./rhoCp_eff_array)*RHS_local[l];
								}
							}
						}
					}
				...
				if (ej == ny-1) {
					...
					if (ctx->bcQT[0].face[face] == VALUE) {
						...
						for (l=0,k = 0; k < ctx->e2D.nphiy; k++){
							for (j = 0; j < ctx->e2D.nphiz; j++) {
								for (i = 0; i < ctx->e2D.nphix; i++, l++) {
									RHS_array[ek+k][ej+1][ei+i] +=  -1.0*(1./rhoCp_eff_array)*RHS_local[l];
								}
							}
						}
					}
				...
				if (ek == nz-1) {
					...
					if (ctx->bcQT[0].face[face] == VALUE) {
						...
						for (l=0,k = 0; k < ctx->e2D.nphiz; k++){
							for (j = 0; j < ctx->e2D.nphiy; j++) {
								for (i = 0; i < ctx->e2D.nphix; i++, l++) {
									RHS_array[ek+1][ej+j][ei+i] +=  -1.0*(1./rhoCp_eff_array)*RHS_local[l];
								}
							}
						}
					}
	...
	ierr = DMDAVecRestoreArray(ctx->daScal,fluxbc_local,&fluxbc_array);CHKERRQ(ierr);
	ierr = DMRestoreLocalVector(ctx->daScal,&fluxbc_local);CHKERRQ(ierr);
...
//	ierr = PetscPrintf(PETSC_COMM_WORLD,"%g,%g,%g,%g,%g,%g \n",kx,ky,kz,kxy,kxz,kyz);CHKERRQ(ierr);
					...
					for (jj = 0; jj < e->nphiy; jj++) {
						for (ii = 0; ii < e->nphix; ii++,l++) {
							...
							for (eg = 0; eg < e->ng; eg++) {
								...
								Kd_ele[l] += ((kx*e->dphi[k][j][i][0][eg]+kxy*e->dphi[k][j][i][1][eg]+kxz*e->dphi[k][j][i][2][eg])*e->dphi[kk][jj][ii][0][eg]
	...
	for (l = 0; l < 64; l++) {
//		ierr = PetscPrintf(PETSC_COMM_WORLD,"l = %d Kd_ele[%d] = %e \n",l,Kd_ele[l]);CHKERRQ(ierr);
	}
...
#undef __FUNCT__
#define __FUNCT__ "VecApplyHeatFluxBC"
extern PetscErrorCode VecApplyHeatFluxBC(PetscReal *RHS_local,PetscReal ***flux_array,PetscInt ek,PetscInt ej,PetscInt ei,FACE face,CartFE_Element2D *e)
{
	PetscErrorCode ierr;
	PetscInt       i,j,k,l,g;
	PetscReal      *flux_elem;
		...
		case X0:
			for (k = 0; k < e->nphix; k++) {
				for (j = 0; j < e->nphiy; j++) {
					for (i = 0; i < e->nphiz; i++) {
						for (g = 0; g < e->ng; g++) {
							flux_elem[g] += e->phi[i][j][k][g]*flux_array[ek+k][ej+j][ei+i];
						}
					}
				}
			}
			...
			for (l=0,k = 0; k < e->nphix; k++) {
				for (j = 0; j < e->nphiy; j++) {
					for (i = 0; i < e->nphiz; i++, l++) {
						RHS_local[l] = 0.;
						for (g = 0; g < e->ng; g++) {
							RHS_local[l] += e->weight[g]*e->phi[i][j][k][g]*flux_elem[g];
						}
					}
				}
			}
		...
		case X1:
			for (k = 0; k < e->nphix; k++) {
				for (j = 0; j < e->nphiy; j++) {
					for (i = 0; i < e->nphiz; i++) {
						for (g = 0; g < e->ng; g++) {
							flux_elem[g] += e->phi[i][j][k][g]*flux_array[ek+k][ej+j][ei+1];
						}
					}
				}
			}
		...
		case Y0:
			for (k = 0; k < e->nphiy; k++) {
				for (j = 0; j < e->nphiz; j++) {
					for (i = 0; i < e->nphix; i++) {
						for (g = 0; g < e->ng; g++) {
							flux_elem[g] += e->phi[j][k][i][g]*flux_array[ek+k][ej+j][ei+i];
						}
					}
				}
			}
			...
			for (l=0,k = 0; k < e->nphiy; k++) {
				for (j = 0; j < e->nphiz; j++) {
					for (i = 0; i < e->nphix; i++, l++) {
						RHS_local[l] = 0.;
						for (g = 0; g < e->ng; g++) {
							RHS_local[l] += e->weight[g]*e->phi[j][k][i][g]*flux_elem[g];
						}
					}
				}
			}
		...
		case Y1:
			for (k = 0; k < e->nphiy; k++) {
				for (j = 0; j < e->nphiz; j++) {
					for (i = 0; i < e->nphix; i++) {
						for (g = 0; g < e->ng; g++) {
							flux_elem[g] += e->phi[j][k][i][g]*flux_array[ek+k][ej+1][ei+i];
						}
					}
				}
			}
		...
		case Z0:
			for (k = 0; k < e->nphiz; k++) {
				for (j = 0; j < e->nphiy; j++) {
					for (i = 0; i < e->nphix; i++) {
						for (g = 0; g < e->ng; g++) {
							flux_elem[g] += e->phi[k][j][i][g]*flux_array[ek][ej+j][ei+i];
						}
					}
				}
			}
			...
			for (l=0,k = 0; k < e->nphiz; k++) {
				for (j = 0; j < e->nphiy; j++) {
					for (i = 0; i < e->nphix; i++, l++) {
						RHS_local[l] = 0.;
						for (g = 0; g < e->ng; g++) {
							RHS_local[l] += e->weight[g]*e->phi[k][j][i][g]*flux_elem[g];
						}
					}
				}
			}
		...
		case Z1:
			for (k = 0; k < e->nphiz; k++) {
				for (j = 0; j < e->nphiy; j++) {
					for (i = 0; i < e->nphix; i++) {
						for (g = 0; g < e->ng; g++) {
							flux_elem[g] += e->phi[k][j][i][g]*flux_array[ek+1][ej+j][ei+i];
						}
					}
				}
			}
...
#undef __FUNCT__
#define __FUNCT__ "HeatMatN"
extern PetscErrorCode HeatMatN(PetscReal *KN_ele,CartFE_Element3D *e,PetscInt ek,PetscInt ej,PetscInt ei, PetscReal ****vel_array)
{
	PetscErrorCode ierr;
	PetscInt i,j,k,l;
	PetscInt ii,jj,kk;
	...
	ierr = PetscMalloc3(e->ng,PetscReal,&dvelx_dx_loc,e->ng,PetscReal,&dvely_dy_loc,e->ng,PetscReal,&dvelz_dz_loc);CHKERRQ(ierr);
	for (eg = 0; eg < e->ng; eg++){
		velx_loc[eg] = 0.;
		vely_loc[eg] = 0.;
		velz_loc[eg] = 0.;
						...
						for (ii = 0; ii < e->nphix; ii++,l++) {
							...
							for (eg = 0; eg < e->ng; eg++) {
								...
								*(velx_loc[eg]*e->dphi[kk][jj][ii][0][eg]
								  +vely_loc[eg]*e->dphi[kk][jj][ii][1][eg]+velz_loc[eg]*e->dphi[kk][jj][ii][2][eg])*e->weight[eg];
							}
						}
					}
				}
...
extern PetscErrorCode VecApplyHeatSourceTerms(PetscReal *K1_local,PetscReal *K2_local,PetscReal ***source_array,CartFE_Element3D *e,PetscInt ek,PetscInt ej,PetscInt ei,VFCtx *ctx, PetscReal ****vel_array)
{
	PetscErrorCode ierr;
	PetscInt       i,j,k,l;
	PetscInt       eg;
	...
	for (eg = 0; eg < e->ng; eg++) {
//		ierr = PetscPrintf(PETSC_COMM_WORLD," loc_source[%d] = %g, dsource_dx_loc[%d] = %g  \n",eg, loc_source[l], eg, dsource_dx_loc[l]);CHKERRQ(ierr);
	}
		...
		for (l = 0,k = 0; k < e->nphiz; k++) {
			for (j = 0; j < e->nphiy; j++) {
				for (i = 0; i < e->nphix; i++,l++) {
					...
					K2_local[l] = 0.;
					for (eg = 0; eg < e->ng; eg++) {
						K1_local[l] += loc_source[eg]*e->phi[k][j][i][eg]*e->weight[eg];
						K2_local[l] += (velx_loc[eg]*dsource_dx_loc[eg]+vely_loc[eg]*dsource_dy_loc[eg]+velz_loc[eg]*dsource_dz_loc[eg])*e->phi[k][j][i][eg]*e->weight[eg];
					}
//					ierr = PetscPrintf(PETSC_COMM_WORLD,"l = %d, K1[%d] = %e, K2[%d] = %e  \n",l, l, K1_local[l], l, K2_local[l]);CHKERRQ(ierr);
...
#undef __FUNCT__
#define __FUNCT__ "HeatMatC"
extern PetscErrorCode HeatMatC(PetscReal *KC_ele,CartFE_Element3D *e,PetscInt ek,PetscInt ej,PetscInt ei, PetscReal ****vel_array)
{
	PetscErrorCode ierr;
	PetscInt i,j,k,l;
	PetscInt ii,jj,kk;
