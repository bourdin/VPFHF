...
extern PetscErrorCode VF_FormFlowStandardFEMMatricesnVectors(Mat K,Mat Krhs,Vec RHS,VFFields * fields,VFCtx *ctx)
{
  ...
  ierr = DMGetLocalVector(ctx->daScalCell,&w_old_local);CHKERRQ(ierr);
  ierr = DMGlobalToLocalBegin(ctx->daScalCell,ctx->widthc_old,INSERT_VALUES,w_old_local);CHKERRQ(ierr);
  ierr = DMGlobalToLocalEnd(ctx->daScalCell,ctx->widthc_old,INSERT_VALUES,w_old_local);CHKERRQ(ierr);
  ierr = DMDAVecGetArray(ctx->daScalCell,w_old_local,&w_old_array);CHKERRQ(ierr);
    ...
    for (ej = ys; ej < ys+ym; ej++) {
      for (ei = xs; ei < xs+xm; ei++) {
        ...
        for (l = 0; l < nrow*nrow; l++) {
          KS_local[l] = m_inv_array[ek][ej][ei]*KS_local[l];
        }
        ierr = MatSetValuesStencil(K,nrow,row,nrow,row,KS_local,ADD_VALUES);CHKERRQ(ierr);
        ierr = VF_MatA_local(KS_local,&ctx->e3D,ek,ej,ei,v_old_array);CHKERRQ(ierr);
        for (l = 0; l < nrow*nrow; l++) {
          KS_local[l] = m_inv_array[ek][ej][ei]*KS_local[l];
        }
        ...
        if(ctx->FlowDisplCoupling && ctx->ResFlowMechCoupling == FIXEDSTRESS){
          ...
          for (l = 0; l < nrow*nrow; l++) {
            KF_local[l] = pow(ctx->matprop[ctx->layer[ek]].beta,2)*KF_local[l]/k_dr_array[ek][ej][ei];
          }
          ierr = MatSetValuesStencil(K,nrow,row,nrow,row,KF_local,ADD_VALUES);CHKERRQ(ierr);
          ierr = VF_MatA_local(KF_local,&ctx->e3D,ek,ej,ei,v_old_array);CHKERRQ(ierr);
          for (l = 0; l < nrow*nrow; l++) {
            KF_local[l] = pow(ctx->matprop[ctx->layer[ek]].beta,2)*KF_local[l]/k_dr_array[ek][ej][ei];
          }
        ...
        if(ctx->FractureFlowCoupling){
          ierr = VF_MatDFractureFlowCoupling_local(KD_local,&ctx->e3D,ek,ej,ei,w_array[ek][ej][ei],v_array);CHKERRQ(ierr);
          for (l = 0; l < nrow*nrow; l++) {
            K1_local[l] = 4.*theta/(12.*mu)*timestepsize*KD_local[l];
            K2_local[l] = -4.*(1.-theta)/(12.*mu)*timestepsize*KD_local[l];
          }
          ierr = MatSetValuesStencil(K,nrow,row,nrow,row,K1_local,ADD_VALUES);CHKERRQ(ierr);
          ierr = MatSetValuesStencil(Krhs,nrow,row,nrow,row,K2_local,ADD_VALUES);CHKERRQ(ierr);
          ierr = VF_RHSFractureFlowCoupling_local(RHS_local,&ctx->e3D,ek,ej,ei,w_array[ek][ej][ei],v_array,w_old_array[ek][ej][ei],v_old_array);
          for (l = 0,k = 0; k < ctx->e3D.nphiz; k++) {
            for (j = 0; j < ctx->e3D.nphiy; j++) {
              for (i = 0; i < ctx->e3D.nphix; i++,l++) {
        ...
        if(ctx->FlowDisplCoupling && ctx->ResFlowMechCoupling == FIXEDSTRESS){
          ...
          for (l = 0,k = 0; k < ctx->e3D.nphiz; k++) {
            for (j = 0; j < ctx->e3D.nphiy; j++) {
              for (i = 0; i < ctx->e3D.nphix; i++,l++) {
                RHS_array[ek+k][ej+j][ei+i] += 1.0*RHS_local[l]/k_dr_array[ek][ej][ei];
              }
            }
          }
        }
        ierr = RemoveModulusEffect(RHS_local,&ctx->e3D,ek,ej,ei,pressure_array,v_array);
        for (l = 0,k = 0; k < ctx->e3D.nphiz; k++) {
          for (j = 0; j < ctx->e3D.nphiy; j++) {
            for (i = 0; i < ctx->e3D.nphix; i++,l++) {
                  ...
                  for (j = 0; j < ctx->e3D.nphiy; j++) {
                    for (i = 0; i < ctx->e3D.nphix; i++,l++) {
                      if(ctx->well[w_no].type == INJECTOR){
                        RHS_array[ek+k][ej+j][ei+i] += timestepsize*RHS_local[l]/w_no2;
                      }
  ...
  ierr = DMDAVecRestoreArray(ctx->daScalCell,w_old_local,&w_old_array);CHKERRQ(ierr);
	ierr = DMRestoreLocalVector(ctx->daScalCell,&w_old_local);CHKERRQ(ierr);
