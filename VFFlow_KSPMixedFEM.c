			...
			for (l=0,k = 0; k < e->nphix; k++) {
				for (j = 0; j < e->nphiy; j++) {
					for (i = 0; i < e->nphiz; i++, l++) {
						RHS_local[l] = 0.;
						for (g = 0; g < e->ng; g++) {
							RHS_local[l] -= kx*beta_c/mu * e->weight[g]*e->phi[i][j][k][g]*(pow(v_elem[g],2))*pre_elem[g];
						}
					}
				}
			}
			...
			for (l=0,k = 0; k < e->nphiy; k++) {
				for (j = 0; j < e->nphiz; j++) {
					for (i = 0; i < e->nphix; i++, l++) {
						RHS_local[l] = 0.;
						for (g = 0; g < e->ng; g++) {
							RHS_local[l] -= ky*beta_c/mu * e->weight[g]*e->phi[j][k][i][g]*(pow(v_elem[g],2))*pre_elem[g];
						}
					}
				}
			}
			...
			for (l=0,k = 0; k < e->nphiz; k++) {
				for (j = 0; j < e->nphiy; j++) {
					for (i = 0; i < e->nphix; i++, l++) {
						RHS_local[l] = 0.;
						for (g = 0; g < e->ng; g++) {
							RHS_local[l] -= kz*beta_c/mu * e->weight[g]*e->phi[k][j][i][g]*(pow(v_elem[g],2))*pre_elem[g];
						}
					}
				}
			}
...
extern PetscErrorCode Flow_Vecf(PetscReal *Kf_ele,CartFE_Element3D *e,PetscInt ek,PetscInt ej,PetscInt ei,PetscInt c,VFFlowProp flowpropty,PetscReal ****perm_array,PetscReal ***v_array)
{
  PetscErrorCode ierr;
	PetscInt  i,j,k,l;
	PetscInt  eg;
	...
	for (l = 0,k = 0; k < e->nphiz; k++) {
		for (j = 0; j < e->nphiy; j++) {
			for (i = 0; i < e->nphix; i++,l++) {
				...
				for (eg = 0; eg < e->ng; eg++) {
					/* Need to multiply by the permability when it is available*/
					Kf_ele[l] += 0.5*(k1*gx+k2*gy+k3*gz)*gamma_c*beta_c*rho/mu*e->phi[k][j][i][eg]*(pow(v_elem[eg],2))*e->weight[eg];
				}
			}
		}
	}
...
extern PetscErrorCode Flow_MatBTranspose(PetscReal *KB_ele,CartFE_Element3D *e,PetscInt ek,PetscInt ej,PetscInt ei,PetscInt c,VFFlowProp flowpropty,PetscReal ****perm_array,PetscReal ***v_array)
{
	PetscErrorCode ierr;
	PetscInt  i,j,k,l;
	PetscInt  ii,jj,kk;
						...
						for (ii = 0; ii < e->nphix; ii++,l++) {
							...
							for (eg = 0; eg < e->ng; eg++) {
								KB_ele[l] += -beta_c/mu*(e->phi[kk][jj][ii][eg]*(k1*e->dphi[k][j][i][0][eg]+k2*e->dphi[k][j][i][1][eg]+k3*e->dphi[k][j][i][2][eg])
														 +0.5*(k1*e->dphi[kk][jj][ii][0][eg]+k2*e->dphi[kk][jj][ii][1][eg]+k3*e->dphi[kk][jj][ii][2][eg])*e->phi[k][j][i][eg])*(pow(v_elem[eg],2))*e->weight[eg];
							}
						}
					}
				}
				...
				for (kk = 0; kk < e->nphiz; kk++) {
					for (jj = 0; jj < e->nphiy; jj++) {
						for (ii = 0; ii < e->nphix; ii++,l++) {
							A_local[l] = 0;
							for (eg = 0; eg < e->ng; eg++) {
								A_local[l] += 0.5*e->phi[k][j][i][eg]*e->phi[kk][jj][ii][eg]*(pow(v_elem[eg],2))*e->weight[eg];
							}
						}
					}
				}
